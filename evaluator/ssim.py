...
def msssim(img1, img2, weights=None, window_size=11, window=None, size_average=True,
           val_range=None, normalize=False, hotfix_nan=True):
    if weights is None:
    ...
    for lv in range(levels):
        sim, cs = ssim(
            img1, img2, window_size=window_size, window=window, size_average=False,
    ...
    # L = n_levles
    mssim = torch.stack(mssim)  # [L] or [L, B]
    mcs = torch.stack(mcs)  # [L]
    ...
    # pow1[:-1]  # [L-1]
    # pow2[-1]  # [B]
    mul = pow1[:-1].view(1, -1) * pow2[-1].view(-1, 1)
    output = torch.prod(mul, dim=1)
